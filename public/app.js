...
/***/ function(module, exports, __webpack_require__) {
	...
	var _baconjs = __webpack_require__(3);

	var _baconjs2 = _interopRequireDefault(_baconjs);

	var _helpers = __webpack_require__(7);

	var _redmine = __webpack_require__(8);

	var Redmine = _interopRequireWildcard(_redmine);

	var _toggl = __webpack_require__(9);

	var Toggl = _interopRequireWildcard(_toggl);
	...
	var projectSelector = (0, _jquery2['default'])('#projet');
	var issuesContainer = (0, _jquery2['default'])('#results');

	var redmineApiKey = (0, _helpers.configuration)((0, _jquery2['default'])('[name=redmine]'), 'redmineApiKey');
	var togglApiKey = (0, _helpers.configuration)((0, _jquery2['default'])('[name=toggl]'), 'togglApiKey');

	var projets = redmineApiKey.filter(_helpers.notEmpty).flatMapLatest(function (apiKey) {
		return _baconjs2['default'].fromPromise(Redmine.APICall(apiKey, '/projects.json', { limit: 100 }));
	}).map('.projects');

	projets.onValue((0, _helpers.updateSelect)(projectSelector));

	var issues = projectSelector.asEventStream('change').map(function (e) {
		return (0, _jquery2['default'])(e.target).val();
	}).combine(redmineApiKey, function (projectId, apiKey) {
		return { projectId: projectId, apiKey: apiKey };
	}).flatMapLatest(function (data) {
		return _baconjs2['default'].fromPromise(Redmine.APICall(data.apiKey, '/issues.json', {
			limit: 100,
	...
	var issuesByVersion = issues.map(Redmine.groupIssuesByVersion);

	function issueItem(issue) {
		var url = Redmine.URL + '/issues/' + issue.id;
		var estimation = issue.estimated_hours || 0;
	...
	function updateIssues(issuesByVersion) {
		...
		for (var versionId in issuesByVersion) {
			var version = issuesByVersion[versionId];
	...
	var refreshTimesForVersion = (0, _jquery2['default'])('body').asEventStream('click', '.get-times').map(function (e) {
		return (0, _jquery2['default'])(e.target).parent().data('id');
	});

	var issuesTimes = refreshTimesForVersion.combine(issuesByVersion, function (versionId, issuesByVersion) {
		return issuesByVersion[versionId].issues.map(function (issue) {
			return issue.id;
		});
	}).combine(togglApiKey, function (issues, togglApiKey) {
		return { issues: issues, togglApiKey: togglApiKey };
	}).flatMap(function (data) {
		return _baconjs2['default'].fromArray(data.issues)
		// .delay(100) TODO Fix me
		.flatMap(function (issueId) {
			var params = {
				description: '#' + issueId
			};
			return _baconjs2['default'].fromPromise(Toggl.reportsAPICall(data.togglApiKey, '/details', params)).map(function (toggl) {
				return { total_grand: toggl.total_grand, total_billable: toggl.total_billable, id: issueId };
			});
		});
	});
	...
	function updateIssuesTime(issue) {
		(0, _jquery2['default'])('#issue-' + issue.id + ' .effectif').html('<strong>' + toHumanDuration(issue.total_billable) + '</strong> / ' + toHumanDuration(issue.total_grand));
	}
...
/***/ function(module, exports) {
	...
	function configuration(field, localStorageKey) {
		var config = field.asEventStream('change').map('.target.value').toProperty(localStorage.getItem(localStorageKey));

		config.onValue(function (key) {
			return localStorage.setItem(localStorageKey, key);
		});
		config.onValue(function (key) {
			return field.val(key);
		});
		return config;
	}
	...
	function updateSelect(elt, valueKey, labelKey) {
		...
		return function (source) {
			...
			source.map(function (item) {
				return elt.append('<option value="' + item[valueKey] + '">' + item[labelKey] + '</option>');
			});
		};
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var URL = 'http://projets.occitech.fr';

	function APICall(apiKey, resource, params) {
		params = params || {};
		params.key = apiKey;

		return _jquery2['default'].get(URL + resource, params);
	}

	function groupIssuesByVersion(issues) {
		return issues.reduce(function (groupedIssues, issue) {
			var version = issue.fixed_version || { id: 'none', name: 'Sans version' };
	...
	exports.URL = URL;
	exports.groupIssuesByVersion = groupIssuesByVersion;
...
/***/ function(module, exports, __webpack_require__) {
	...
	function reportsAPICall(apiKey, resource, params) {
		var url = 'https://toggl.com/reports/api/v2';
	...
	exports.reportsAPICall = reportsAPICall;
